...
class LrUpdaterHook(Hook):
    ...
    def get_regular_lr(self, runner):
        if len(self.base_lr) != len(runner.optimizer.param_groups):
...
class LogLrUpdaterHook(LrUpdaterHook):

    def __init__(self, end_lr, num_epochs, **kwargs):
        self.end_lr = end_lr
        self.num_epochs = num_epochs
        super(LogLrUpdaterHook, self).__init__(**kwargs)

    def get_lr(self, runner, base_lr):
        if runner.epoch >= self.num_epochs:
            return self.end_lr
        lr_spaces = logspace(log10(base_lr), log10(self.end_lr), self.num_epochs)
        return lr_spaces[runner.epoch]
