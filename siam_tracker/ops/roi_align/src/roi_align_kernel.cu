...
__global__ void ROIAlignForward(const int nthreads, const scalar_t *bottom_data,
    ...
    CUDA_1D_KERNEL_LOOP(index, nthreads) {
        ...
        for (int iy = 0; iy < sample_num_h; iy++) {
            ...
            const scalar_t y = roi_start_h + ph * bin_size_h +
                static_cast<scalar_t>(iy+1) * bin_size_h / static_cast<scalar_t>(sample_num_h+1);
            for (int ix = 0; ix < sample_num_w; ix++) {
                ...
                const scalar_t x = roi_start_w + pw * bin_size_w +
                    static_cast<scalar_t>(ix+1) * bin_size_w / static_cast<scalar_t>(sample_num_w+1);
      ...
      features.type(), "ROIAlignLaucherForward", ([&] {
        ...
        ROIAlignForward<scalar_t>
            <<<GET_BLOCKS(output_size), THREADS_PER_BLOCK>>>(
                ...
                sample_num, num_channels, data_height, data_width, pooled_height,
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
        ...
        ROIAlignBackward<scalar_t>
            <<<GET_BLOCKS(output_size), THREADS_PER_BLOCK>>>(
                ...
                num_channels, data_height, data_width, pooled_height, pooled_width,
                bottom_diff);
      }));
