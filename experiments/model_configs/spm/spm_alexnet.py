...
work_dir = './output/spm/spm_alexnet/'
log_level = 'INFO'
resume_from = None
...
model = dict(
    ...
    fm_fusion=dict(
        type='BoxAlign',
        feat_name=['conv2', 'conv4'],
        in_channels=[192, 256],
        stride=[8, 8],
        out_size=6,
        post_convs=dict(num_layers=1, in_channels=(192+256)*2, out_channels=256, kernel_size=1, nonlinear_last=True),
        fuse_type='concat',
    ),
    fm_head=dict(
        type='CompareHead',
